...
# Change basis and functional for a 2-site model
...
for state in s:
    ...
    qc_wb97mv = state + '_wb97m-v.in'
    f_wb97mv = open(qc_wb97mv, 'w')
    j = re.sub("wb97x\-d", "wb97m-v", i, flags = re.DOTALL)
    f_wb97mv.write(j)
    f_wb97mv.close()

    qc_wb97xd_simple = state + '_wb97x-d_simple.in'
    ...
    qc_wb97mv_simple = state + '_wb97m-v_simple.in'
